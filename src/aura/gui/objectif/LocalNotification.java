...
/**
 * <p>
 * Local notifications are user notifications that are scheduled by the app
 * itself. They are very similar to push notifications, except that they
 * originate locally, rather than remotely.</p>
 *
 * <p>
 * They enable an app that isnt running in the foreground to let its users know
 * it has information for them. The information could be a message, an impending
 * calendar event, or new data on a remote server. They can display an alert
 * message or they can badge the app icon. They can also play a sound when the
 * alert or badge number is shown.
 * </p>
 * <p>
 * When users are notified that the app has a message, event, or other data for
 * them, they can launch the app and see the details. They can also choose to
 * ignore the notification, in which case the app is not activated.
 * </p>
 *
 * <p>
 * This class encapsulates a single notification (though the notification can be
 * repeating).
 *
 * <h3>Usage</h3>
 ...
 *
 * <p>
 * <strong>Android Note:</strong> The default image that is used on the android
 * top status bar and on the notification itself is the App's icon. However
 * Android 5 and above will only display this image as a silhouette using alpha
 * pixels. This will result in many icons appearing to be a blank white square.
 * In such cases you can provide an alternate image to be displayed instead.
 * Place a 24x24 image named "ic_stat_notify.png" in your project's
 * {@literal native/android} directory, and this image will be used instead.
 * </p>
 *
 * @author shannah
 * @see com.codename1.ui.Display#scheduleLocalNotification(LocalNotification n,
...
public class LocalNotification implements LocalNotificationCallback {
     ...
     * should refer to a sound file that is bundled in the default package of
     * your app.
     *
     ...
     * should refer to a sound file that is bundled in the default package of
     * your app. The name of the file must start with the "notification_sound"
     * prefix.
     *
     * <code><pre>
     ...
     * }
     * so you can use it as a lookup key to retrieve the rest of the information
     * as required from storage or some other mechanism.
     *
     ...
     * ignored.
     *
     * @param image a path to the image, the image needs to be placed in the app
     * root.
     */
